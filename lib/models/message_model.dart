...
class MessageModel {
  ...
  /// First-frame JPEG of a video message, base64, generated by the sender at
  /// pick time (see `_generateVideoThumbnail` in chat_screen.dart) and carried
  /// inline in the encrypted envelope — so the receiver renders the poster
  /// frame the moment the message arrives, before (or without) downloading the
  /// clip. Null for every non-video message and for videos sent before this
  /// shipped; the bubble then falls back to a plain play-button placeholder.
  /// A small still, not the clip — the video itself lives in Storage and only
  /// its URL travels in [mediaUrl].
  final String? videoThumbnailBase64;
